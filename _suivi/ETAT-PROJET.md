...
>
> Dernière réécriture : **24/08/2026** (dernière passe : deuxième audit de `t0` —
> les images livrées, le QCM de bilan de la séance 2, l'entraide retirée des pages
> élèves, et les vidéos qui ne partent plus chez Google avant le clic)
...
| **SNT — `t0` Introduction** | 🔄 **Refondue le 23/08/2026, auditée le 24/08.** Elle enseigne les mécanismes du cours, et c'était la seule à ne pas les avoir. État courant : **3 séances + 1 défi de classe · 24 étapes · 16 portes · 6 QCM (39 questions) · 4 réponses rédigées corrigées · 2 réponses personnelles · 3 dépôts d'image · 1 tri · 1 entrée de glossaire · 1 poste de visionnage · 4 SVG maison**. Apports du 24/08 : les **cinq photos manquantes** de l'accroche (valideur Navigo, caisse automatique, lave-linge, montre connectée, baie de serveurs — Wikimedia, CC, attribution complète) avec une lecture en trois temps *entrée / calcul / action* sous **chacune** des huit vignettes ; les **26 images du dossier réextraites du PDF source avec leur masque de transparence** — elles étaient aplaties sur du noir ; une **vidéo de dix minutes** en 2.2 avec prise de notes, et la séance 2 renumérotée 2.1→2.7 ; l'étape « autopsie » remplacée par un **QCM de bilan de 15 questions, dont six avec photo** ; l'**entraide retirée** des pages élèves ; le **défi débranché** devenu un défi de reconnaissance avec dépôt ; les deux **languettes PCIe annotées** en surcouche SVG ; les broches VGA et DVI recentrées. Vérifié au navigateur : **0 erreur JS**, la séance 2 se termine réellement (modale 🎉), les images du QCM tiennent sur 1366×768 **et** sur iPad portrait, la fiche de séance embarque ses 18 figures deux par ligne, et **aucune requête réseau ne part avant un clic**. ⏳ **Rien n'est validé** : tout le contenu reste une proposition. Restent ouverts : les captures d'interface de l'étape 1.1 (lot suspendu) et la **licence des 37 images issues du PDF d'un collègue**.
...
| **SNT — moteur partagé (`sequence-snt.js` v40)** | 🔧 **Quatre changements le 24/08**, tous vérifiés sur `t0`, `t1`, `t2` et `m1`. ① **Les vidéos ne partent plus chez Google avant le clic** : une `<iframe>` YouTube, même en `nocookie` et même dans une étape masquée, contactait `fonts.gstatic.com`, `google.com` et `googleapis.com` **dès l'ouverture de la page**. Les pages déclarent désormais `data-src`, et `initVideos()` pose une **affiche locale** ; mesuré à zéro requête externe sur les quatre pages. ② La **barre de fiche** est masquée tant que la dernière étape à valider de la séance n'est pas dévoilée (elle était injectée sans condition). ③ La pastille **« rendu, pas encore relu »** ne s'affiche plus verte pleine dans la barre de progression alors qu'elle est creuse et pointillée dans le fil : l'ordre des tests était inversé. ④ La **fiche de séance embarque les figures** des étapes à valider, avec leur légende, deux par ligne à l'impression — les surcouches d'annotation, décoratives, en sont exclues. Les deux blocs du fichier restant cloisonnés, la révélation d'une étape prévient la barre de fiche par un événement `etape-revelee`. **Rappel du 23/08** : `data-essais-avant-correction` et `data-valide-sur-interaction` restent en place, sans effet par défaut.
