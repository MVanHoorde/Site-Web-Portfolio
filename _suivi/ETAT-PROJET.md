...
>
> Dernière réécriture : **23/08/2026** (dernière passe : refonte complète de la
> séquence d'introduction `t0` — portage sur le moteur, trois séances, contenu
> des documents source porté en entier)
...
| **SNT** | 8 séquences de thème (`t0`→`t7`) **+ 1 module transversal** (`m1` « Représenter l'information »). `t0`, `t1` et `t2` sont sur le **moteur partagé** et en **V0 complète** ; `t3`→`t7` en V0 partielle (S1 rédigée, suite en 🚧). **`t1` est le seul thème validé sur le fond** (23/08) — il sera *clos* après la vérification des fiches. **`t0` a été refondue le 23/08** : voir sa ligne. |
...
| **SNT — `t0` Introduction** | 🔄 **Refondue le 23/08/2026.** Elle enseigne les mécanismes du cours, et c'était la seule à ne pas les avoir : elle tournait sur un **fork figé du moteur** (46 % du fichier) et ne portait que **43 %** du contenu de ses documents source. Désormais : **3 séances + 1 débranchée · 23 étapes · 16 portes · 5 QCM (31 questions) · 4 réponses rédigées corrigées · 2 réponses personnelles · 2 dépôts d'image · 1 tri · 1 entrée de glossaire · 43 des 47 images placées · 4 SVG maison** (planche de 14 connecteurs, slots PCIe, pouces et diagonale, définition contre résolution). Le `setTimeout` qui simulait la correction est mort ; les rédigés partent réellement en base ; le mode enseignant demande un **code** et se coupe à 30 min. L'étape 1.1 présente les **douze mécanismes** du dispositif, chacun **mis en action** dans une étape suivante. Vérifié au navigateur : **0 erreur JS**, cascade de déverrouillage s1→s2→s3, bilan masqué, tri, dépôt, glossaire, fiche contenant les réponses. ⏳ **Rien n'est validé** : tout le contenu est une proposition, cinq images manquent (l'accroche de la séance 1), et la recherche en ligne de l'étape 2.4 attend un arbitrage. |
