...
| Date | Décision | Statut |
|---|---|---|
| 20/09/2026 | 🔴 **O7 n'enseigne plus qu'une méthode : l'opération inverse.** Le triangle devient une **mention de cinq lignes**, sans dessin ni mode d'emploi&nbsp;; la « méthode des chiffres » et les « cinq niveaux » disparaissent. Décision pédagogique de Loïc, étayée par l'audit du 20/09 (Southall 2016 ; Koenig, RSC ; Skemp 1976)&nbsp;: le triangle **remplace** l'algèbre au lieu de la soutenir, ne vaut que pour `a = b × c`, et empêche le transfert vers les maths. **Remplace O-26** (les cinq niveaux), désormais sans objet. | ✅ en vigueur |
| 20/09/2026 | 🔴 **La mention du triangle n'est pas sur la fiche A4.** Elle n'a d'intérêt qu'**une fois**, en ligne, au moment où l'élève se demande ce qu'est devenu ce qu'il a appris au collège. La fiche est ce qu'il colle et relit toute l'année&nbsp;: elle ne porte que la méthode. Contrôle&nbsp;: `grep -i triangle fiches/fiche-2nde-o7-*.html` ne rend que des commentaires. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Les exercices de O7 se font sur la feuille, jamais dans un champ.** 71 relations, un bouton « Afficher la correction » sous chacune, corrigé **ligne par ligne avec l'opération en marge**. Aucun menu déroulant, aucune saisie de formule, aucun analyseur d'expression. Seule exception&nbsp;: la série (i), où **nommer l'erreur EST l'exercice** — un QCM de quatre propositions s'y justifie. Fondé sur l'effet de test (Roediger & Karpicke 2006) et le feedback différé (Butler & Roediger 2008). | ✅ en vigueur |
| 20/09/2026 | **Les 10 séries de O7 sont classées par STRUCTURE ALGÉBRIQUE, pas par difficulté croissante.** Produit · quotient (numérateur) · quotient (dénominateur) · somme · parenthèse · carré et racine · enchaînements · deux relations · copies fautives · défi. Motif&nbsp;: l'élève qui s'est planté sur un dénominateur vient chercher **la série (c)**, pas « le niveau 2 ». | ✅ en vigueur |
| 20/09/2026 | 🔴 **Aucun corrigé de O7 n'est publié sans contrôle numérique.** Pour chaque item&nbsp;: on fixe les variables indépendantes, la relation de départ est vraie par construction, et on vérifie que la formule du corrigé redonne la grandeur isolée — sur **trois jeux de valeurs**. Les huit copies fautives sont contrôlées à l'envers (leur formule doit **échouer**). 71/71 au vert. Script jetable, hors dépôt. | ✅ en vigueur |
| 20/09/2026 | 🔴 **O8 devient trois pages, et la numérotation des outils reste 1 → 8.** `o8` garde son URL et devient le tronc commun « à la main »&nbsp;; `o8b` (tableur) et `o8c` (calculatrice) sont des **pages sœurs**, absentes du hub, où l'on n'entre que depuis O8 — qui est leur **prérequis affiché**. Motif&nbsp;: un seul outil aurait noyé un élève de quinze ans seul chez lui, et `CONSIGNES-outil-PC.md` §2 impose deux sections, jamais plus. Contrepartie acceptée&nbsp;: le tableau de bord affiche **dix entrées pour huit outils**. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Une seule fiche A4 pour les trois pages de O8**, celle du tronc commun. Ce qui se colle au cahier, c'est la méthode&nbsp;; un mémo « suites de touches » sur papier serait périmé au premier changement de système d'exploitation. | ✅ en vigueur |
| 20/09/2026 | **La fiche O8 passe à trois pages** — dérogation à la règle des deux pages (`CONSIGNES-outil-PC.md` §7), déclarée dans `EXCEPTIONS` de `exporter-fiches.mjs` comme celle de `o3`. Motif&nbsp;: le cours a doublé, et le §7 interdit qu'une partie du cours vive **seulement à l'écran**. Si la dérogation est refusée, c'est la **grille vierge** du verso qui sortira, pas le cours. | ⏳ à valider par Loïc |
| 20/09/2026 | 🔴 **Le jeu de données de O8 vit à un seul endroit** : `assets/js/o8-donnees.js`, chargé par les trois pages. **Pente, ordonnée, R² et interpolations y sont calculés**, jamais écrits&nbsp;: une valeur affichée ne peut pas diverger du tableau qu'a l'élève. 🔴 **Nouvel asset partagé** — le modifier oblige à incrémenter `?v=N` dans les **trois** pages. Il n'est pas encore couvert par le contrôle d'alignement de `verifier.mjs` (hors périmètre, §11). | ✅ en vigueur |
| 20/09/2026 | 🔴 **Aucune séquence de touches n'est inventée, ni extrapolée d'un modèle à l'autre.** Chaque étape de O8c porte `data-verifie="oui|non"` et sa **source en commentaire HTML**. Sont `non`&nbsp;: **TI-82 Advanced** (7 étapes, aucune source propre), **fx-92 Collège** (7 étapes), la **régression proportionnelle** sur TI et sur Casio, et l'**interface de la Graph Math+**. Chaque parcours non vérifié le dit **à l'élève** et renvoie au simulateur NumWorks. Motif&nbsp;: une procédure fausse fait perdre vingt minutes à un élève seul chez lui — c'est pire qu'une procédure absente. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Excel pour le web n'a pas de courbe de tendance sur un nuage de points**, vérifié au 20/09/2026 auprès de la documentation Microsoft. Son parcours **ne fait pas semblant**&nbsp;: il l'annonce dès la première ligne et enseigne le contournement (`=PENTE`, `=ORDONNEE.ORIGINE`, `=COEFFICIENT.DETERMINATION`, puis la droite modèle en seconde série de deux points). C'est le cas de la majorité des élèves via l'ENT&nbsp;: ce parcours est **aussi complet** que celui d'Excel de bureau. | ✅ en vigueur — à re-tester chaque année scolaire |
| 20/09/2026 | **Le sélecteur de matériel est gardé en `localStorage`, pas en base.** « J'ai une NumWorks » est une **préférence d'affichage**, pas une réponse d'élève&nbsp;: rien à enregistrer sur un serveur, rien à montrer au professeur, donc rien qui sorte du navigateur. Tous les accès sont sous `try/catch`&nbsp;: en navigation privée, la page fonctionne et oublie le choix. | ✅ en vigueur |
| 20/09/2026 | **L'auto-évaluation des 71 items est codée en local, et écrit sur une clé à part.** `CONSIGNES-outil-PC.md` §5 interdit de toucher `sequence-snt.js` (22 pages)&nbsp;; le composant n'existait pas. Il est donc dans le `<script>` de la page, et persiste via `Progression.ecrire('cours', 'autoeval-pc-o7', …)` — **clé distincte** de celle du moteur, qui fusionne au premier niveau et relit avant d'écrire&nbsp;: deux composants sur la même clé peuvent se croiser et en perdre un. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Les captures d'écran suivent la règle de leur éditeur, et le repérage se fait en HTML.** Microsoft impose une capture **non modifiée** (redimensionnement seul), **pas de portion**, et la mention « Used with permission from Microsoft. » en légende. Conséquence appliquée à **toutes** les captures&nbsp;: les pastilles numérotées sont positionnées **au-dessus** de l'image en CSS (composant `.capture-annotee`, coordonnées en pourcentage dans un attribut `data-`), jamais dessinées dedans. Crédits&nbsp;: NumWorks, © Texas Instruments, © CASIO selon l'origine. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Aucun appel commercial sur O8b et O8c.** NumWorks publie Epsilon sous licence **non commerciale**, TI et Casio tolèrent l'usage pédagogique « non commercial »&nbsp;: pas de bandeau, pas de lien vers une offre sur ces deux pages. | ✅ en vigueur |
...
| Date | Décision | Statut |
|---|---|---|
| 20/09/2026 | 🔴 **Les photos de matériel ne remplacent pas les schémas au trait, elles s'y ajoutent.** Sur O4, chaque vignette porte désormais la **photo** de l'objet réel **et** son schéma normalisé. Motif&nbsp;: le tracé au trait est du **contenu de cours** dans cet outil-là — l'étape 1.5 enseigne les règles du schéma normalisé — tandis que la photo montre l'objet que l'élève aura en main. | ✅ en vigueur |
| 20/09/2026 | **Origine des images de `2nde-pc-o4/`&nbsp;: `openai-ia`**, images générées par l'IA d'OpenAI d'après des descriptions de Loïc. Crédit affiché en pied de page, détail par fichier dans `assets/img/pc/2nde-pc-o4/CREDITS.md`. 🔴 **Ne jamais effacer les métadonnées** de ces fichiers&nbsp;: elles portent le marqueur de provenance. Le recadrage du 20/09 les a préservées, et l'a vérifié fichier par fichier. | ✅ en vigueur |
| 20/09/2026 | **Pas de plafond de 480 000 px dans `verifier.mjs`.** Cette limite est une clause de la licence **Canva**&nbsp;; aucune image du dépôt n'en vient. À la place, un contrôle utile&nbsp;: **toute image de `assets/img/**` doit être nommée dans le `CREDITS.md` de son dossier**. Le jour où une ligne `canva-design` apparaîtra, c'est là qu'il faudra ajouter le plafond — le commentaire du contrôle le dit. | ✅ en vigueur — remplace la demande du brief du 20/09 |
| 20/09/2026 | 🔴 **Une image générée qui porte une échelle fausse ne rentre pas.** Deux images déposées ont été écartées pour cette seule raison&nbsp;: la **fiole jaugée** (col entièrement gradué au lieu d'un trait unique, &laquo; 70 &raquo; deux fois) et le **verre à pied** (&laquo; 300 &raquo; deux fois). Un pictogramme faux est une erreur de cours&nbsp;: les figures O4-04 et O4-05 restent des **cadres de réservation**. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Les huit rubriques du compte rendu ne bougent pas.** Le brief demandait de scinder &laquo; protocole &raquo; et &laquo; expérience &raquo; en deux rubriques&nbsp;; la frise de O5 vient du document d'une collègue, et la décision O-27 inscrite dans la page interdit d'ajouter ou de retirer une rubrique. La distinction est donc portée par le **texte** de l'étape 1.2 (deux colonnes, &laquo; ce que tu prévois &raquo; / &laquo; ce que tu as fait &raquo;), sans toucher à la liste. | ⏳ à confirmer par Loïc |
| 20/09/2026 | **Le CSS des cadres de réservation reste local aux pages.** Le brief demandait une classe `.fig-vide` dans `assets/css/sequence-snt.css`&nbsp;; `CONSIGNES-outil-PC.md` §5 interdit de toucher cet asset, chargé par 22 fichiers. On réutilise le patron **`.reserve`** de `2nde-pc-o3`, recopié dans le `<style>` de chaque page. | ✅ en vigueur — remplace la demande du brief du 20/09 |
| 20/09/2026 | ✅ **L'outil 6 s'ouvre aux élèves et aux collègues.** Décision de Loïc le 20/09. Lien rétabli au hub, `guide-ap.html` révisé (six outils ouverts, date au 20 septembre), PDF des quatre guides régénérés. 🔴 **Effet de bord** : le renvoi d'O6 vers O7 rend `o7` atteignable alors que le hub l'annonce encore en travaux — à trancher. | ✅ en vigueur |
| 20/09/2026 | 🔴 **Sur la fiche O5, les règles du tableau de mesures, pas le tableau.** Le verso était plein ; le resserrement de forme atteignait la limite de lisibilité. On garde sur le papier les règles (unité en tête de colonne, ce qu'on fait varier d'abord, un titre, deux essais) et le **modèle rempli reste sur la page en ligne**, où la place ne manque pas. Même logique pour les **tolérances de classe A**, absentes de la fiche O4. | ✅ en vigueur |
| 20/09/2026 | **Sur o4 et o5, seuls les espacements du verso ont été resserrés** pour accueillir les ajouts : aucun texte de Loïc n'a été retiré ni réécrit. Contrôle : chaque `.feuille` mesure exactement la hauteur d'une page A4, et `exporter-fiches.mjs` confirme 2 pages à `209,9 × 297,0 mm`. | ✅ en vigueur |
| 20/09/2026 | **Les trois planches de cartes ne vont pas sur la page.** Elles répétaient mot pour mot les grilles de vignettes qui les précédaient. Elles restent dans `assets/img/pc/2nde-pc-o4/` pour la **fiche A4**, où une planche compacte à imprimer a du sens. | ✅ en vigueur |
...
| Date | Décision | Statut |
|---|---|---|
| 13/09/2026 | **Le tableau de bord s'organise en espaces** : SNT · ES 1re · ES Terminale · PC 2nde · CFA, chacun avec ses classes. Pas de rôle administrateur : **un enseignant voit les espaces où il a une classe rattachée**. Les collègues, rattachés à des groupes SNT seulement, n'ont qu'un espace et **arrivent directement sur le SNT**, sans page d'accueil. Même adresse que le tableau de bord déjà envoyé. | ✅ en vigueur — remplace RE-4 |
| 13/09/2026 | **Un compte élève, plusieurs classes.** L'élève garde son identifiant SNT et rejoint la 2nde 1 ou l'AP avec un second code. **Un seul groupe SNT par élève** (deux plafonds et deux correcteurs sinon) ; en PC, 2nde 1 et AP se cumulent. | ✅ en vigueur (`018`) |
| 13/09/2026 | **Nouvelles classes** : `ES1R02` (1re 2), `EST303` (Terminale 3), `EST606` (Terminale 6), `PC2S01` (2nde 1), `AP2S26` (AP de PC, élèves d'origines diverses). Libellés sans nom d'enseignant ni classe d'origine pour l'AP. **Fermées à l'inscription** jusqu'à la mise en ligne du tableau de bord et du client élève. | ✅ en vigueur (`018`) |
| 13/09/2026 | **Ce qui se suit par espace.** ES : progression et réponses, **sans file de correction** (corrections en classe). PC 2nde, classe : outils, consultation des chapitres, téléchargements. PC 2nde, **AP : les outils seulement**. CFA : le livret, tel qu'il est enregistré depuis le 19/08. SNT : inchangé. | ✅ en vigueur |
| 13/09/2026 | **Consultation des chapitres PC : aucune restriction d'accès** — l'élève révise où il veut, c'est son autonomie. Ce qui s'enregistre : ouverture datée, sections débloquées, clics de téléchargement — **pas de temps passé sur la page**. Indiquer « où on en est » dans la progression : à réfléchir plus tard. | ✅ en vigueur · ⏳ repère de progression à décider |
| 13/09/2026 | 🔴 **Convention de clés : une clé de séquence commence par le préfixe de sa famille** — `snt-`, `pc-`, `es1-` / `est-`, `cfa-`. C'est ce préfixe qui dit à la base quel enseignant peut lire la ligne (`famille_de_cle()`). Une page de PC à clé mal préfixée serait lue comme du SNT, donc visible des collègues. Conséquence : **les séquences d'ES s'appelleront `es1-tN-cN` et `est-tN-cN`**, pas `es-tN-cN` (1re T2-C1 et Terminale T2-C1 se seraient confondues). | ✅ en vigueur |
...
| 12/09/2026 | ✅ **D5 — Le branchement en base de l'ES de terminale** | **Tranché le 13/09 : en même temps que la 1re**, au lot 3 du tableau de bord à espaces, avec les clés `est-tN-cN`. Les six gestes sont listés au §4 de `es1-verification.md`. |
...
| RE-4 | ~~**Le tableau de bord est un outil de SNT, et rien d'autre.** Le livret CFA et le reste restent dehors : leur progression ne s'y suit pas. Aucun filtre n'a été codé pour ça — une classe rattachée à personne est invisible de tous, et c'est suffisant~~ | ~~✅~~ remplacée le 13/09/2026 par « Le tableau de bord s'organise en espaces » |
...
| ~~O-26~~ | ~~**`o7` garde les cinq niveaux de la fiche de Loïc.**~~ — **Sans objet depuis le 20/09/2026** : `o7` n’enseigne plus qu’une méthode, et les cinq niveaux ont disparu avec la refonte. Voir le bloc « Refonte des outils O7 et O8 » du 20/09. | ~~remplacée~~ |
